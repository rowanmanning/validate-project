...
module.exports = class NodeProject extends BaseProject {
	...
	async test() {
		...
		await this.manifest.assertPropertyMatches('scripts.prepare', 'husky');
		...
		if (await this.manifest.hasProperty('devDependencies.eslint')) {
			...
			await this.manifest.assertPropertyExists(
				'devDependencies.@rowanmanning/eslint-config',
				null,
				false
			);
			...
			this.npmGroupScripts.verify.push('npm run verify:eslint');
		}
		...
		if (await this.manifest.hasProperty('devDependencies.typescript')) {
			...
			await this.typescriptConfig.assertPropertyMatches('compilerOptions.declaration', true);
			await this.typescriptConfig.assertPropertyMatches(
			...
			await this.gitignore.assertIncludes('*.d.ts');
			await this.gitignore.assertIncludes('*.d.ts.map');

			await this.manifest.assertPropertyMatches(
				'scripts.verify:types',
				'tsc --noEmit --project ./jsconfig.json'
			);
			this.npmGroupScripts.verify.push('npm run verify:types');
		}
		...
		if (await this.manifest.hasProperty('devDependencies.mocha')) {
			...
			if (await this.manifest.hasProperty('devDependencies.eslint')) {
				...
				await this.testEslintConfig.assertPropertyIncludes('extends', '../.eslintrc.json');
			...
			if (await this.fs.directoryExists('test/unit')) {
				...
				this.npmGroupScripts.test.push('npm run test:coverage');

				// Nyc config
				await this.manifest.assertPropertyMatches('nyc.eager', true);
				await this.manifest.assertPropertyMatches('nyc.reporter', ['html', 'text']);
			...
			if (await this.fs.directoryExists('test/integration')) {
				...
				this.npmGroupScripts.test.push('npm run test:integration');
			}
		...
		// Build, verify and test scripts
		if (this.npmGroupScripts.build.length) {
			await this.manifest.assertPropertyMatches(
				'scripts.build',
				this.npmGroupScripts.build.join(' && ')
			);
		}
		if (this.npmGroupScripts.verify.length) {
			await this.manifest.assertPropertyMatches(
				'scripts.verify',
				this.npmGroupScripts.verify.join(' && ')
			);
		}
		if (this.npmGroupScripts.test.length) {
			await this.manifest.assertPropertyMatches(
				'scripts.test',
				this.npmGroupScripts.test.join(' && ')
			);
		}
		...
		if (ciTasks.length) {
			await this.fs.assertFileMatches(
		...
		await this.manifest.assertPropertyMatches('scripts.project:fix', this.runner.fixCommand);
	}

	async minimumNpmVersion() {
		return semver.minVersion((await this.manifest.getProperty('engines.npm')) || '8.x').major;
	}
	...
	async minimumNodeVersion() {
		return semver.minVersion((await this.manifest.getProperty('engines.node')) || '18.x').major;
	}

	async stableNodeVersion() {
		return 18;
	}
	...
	async supportedMongoDbVersions() {
		...
		if (mongodbEngines && semver.validRange(mongodbEngines)) {
			...
			for (
				let version = minimumMongoVersionMajor;
				version <= maximumMongoVersionMajor;
				version += 1
			) {
				versions.push(version);
			}
