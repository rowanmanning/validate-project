...
module.exports = ({hasBuildScript, maximumNodeVersion, packageName}) => `name: Create releases and publish to npm
on:
  push:
    branches:
      - main
  ...
  release-please:
    ...
    steps:
      ...
      - uses: actions/setup-node@v3
        ...
        if: \${{ steps.release.outputs.release_created }}
      - run: npm ci
        if: \${{ steps.release.outputs.release_created }}${hasBuildScript ? `
      - run: npm run build
        if: \${{ steps.release.outputs.release_created }}` : ''}
