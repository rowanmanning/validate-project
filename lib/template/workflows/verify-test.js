...
const verifyBlock = ({ stableNodeVersion }) => `
  # Verify code
  verify:
    name: Verify code
...
const testIntegrationBlock = ({ tasks, supportedNodeVersions, supportedMongoDbVersions }) => {
	...
	const mongoVersionMatrix = (supportedMongoDbVersions || [])
		.map((version) => `'${version}'`)
		.join(', ');
	return `
  ...
  test-integration:
    ...
    strategy:
      matrix:
        node: [${nodeVersionMatrix}]${
			supportedMongoDbVersions?.length
				? `
