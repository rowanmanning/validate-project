...
const fileIsExecutable = require('../utils/file-is-executable');
const IgnoreFile = require('./file/ignore');
const JsonFile = require('./file/json');
const listFiles = require('../utils/list-files');
const makeFileExecutable = require('../utils/make-file-executable');
...
module.exports = class FileSystem {
	...
	get files() {
		if (!this._caches.files) {
			this._caches.files = this.list.then((files) =>
	...
	get directories() {
		if (!this._caches.directories) {
			this._caches.directories = this.list.then((files) =>
				files
	...
	async fileIsExecutable(file) {
		if (!(await this.fileExists(file))) {
			return false;
		}
		return fileIsExecutable(this.runner.path(file));
	}
	...
	async assertFileIsExecutable(file, fix = null) {
		if (!(await this.fileIsExecutable(file))) {
			this.logIssue(
				`File "${file}" must be executable`,
				fix ??
					(async () => {
						await makeFileExecutable(this.runner.path(file));
						return `Changed permissions of file "${file}" to 755`;
					})
			);
		}
	}
	...
	async assertDirectoryNotExists(directory, fix = null) {
		if (await this.directoryExists(directory)) {
			this.logIssue(
				`Directory "${directory}" must not exist`,
				fix ??
					(async () => {
